...
const qualities = ["Excellent", "Good", "Fair", "Poor", "Worst"];

const Page = () => {
  const { signedInUser } = useAppSelector(selectAuth);
  const [fetchingAssessents, setFetchingAssessments] = useState<boolean>(false);
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        const assessments = await fetchAsync(
          "/assessment",
  ...
  if (fetchingAssessents) return <Skeleton count={4} className="h-40" />;

  if (!assessments) {
    // toast.error("Error fetching data");
    return;
  }
  const { mental_score, stress_level, mood, sleep_quality } = assessments;
    ...
    <main className="pb-20">
        ...
        <div className="flex gap-5 md:h-full h-[15em] overflow-x-scroll md:overflow-hidden md:flex-wrap md:justify-center">
          ...
          <div className="h-full p-3 text-white bg-[#2ECC71] rounded-xl">
            ...
            <p className="text-2xl font-bold">
              {stress_level > 3 ? "High stress" : "Low stress"}
            </p>
      ...
      <section className="p-10 pt-0 w-full md:w-[70%] mx-auto">
        ...
        <div className="p-10 w-full max-w-[25em] mx-auto py-12 my-7 text-[#707070] text-2xl rounded-3xl">
          ...
          <Image
            className="relative left-[90%] mt-2"
            src={quoteIcon}
            width={40}
            height={40}
            alt="quote"
